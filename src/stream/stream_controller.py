...
def start_stream(quality):
    ...
    cmd = [
        ...
        "--twitch-low-latency",          # Low-Latency Modus
        "--hls-live-edge", "2",          # kleiner Buffer für Low-Latency
...
def monitor_stream_output():
    """Überwacht Streamlink-Ausgabe, erkennt Werbung (nur Logging)."""
    global streamlink_process, ad_active
    ...
    try:
        for line in iter(streamlink_process.stdout.readline, ""):
            ...
            if m:
                ...
                if not ad_active:
                    print(f"[controller] Werbung erkannt ({duration}s) – Streamlink überspringt automatisch.", flush=True)
                    ad_active = True
            else:
                if ad_active:
                    print("[controller] Werbeblock beendet – weiter wie gewohnt.", flush=True)
                    ad_active = False
...
def main():
    ...
    try:
        while True:
            # Nur wenn der Prozess beendet ist, neu starten
            if streamlink_process and streamlink_process.poll() is not None:
                print("[controller] Streamlink-Prozess beendet → Neustart.", flush=True)
                stop_stream()
                streamlink_process = start_stream(current_quality)
